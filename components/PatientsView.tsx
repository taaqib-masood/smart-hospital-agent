  ...
  return (
    <div className="space-y-6 max-w-[1200px] mx-auto h-[calc(100vh-140px)] flex flex-col">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="text-2xl font-bold text-[#0F172A]">Patient Contacts</h2><p className="text-sm text-slate-500 mt-0.5">Reception contact details, visit count, and operational notes.</p></div>
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-3.5 py-2 bg-[#00685f] text-white text-xs font-bold rounded-lg"><Plus size={14} /> Add Contact</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0">
        <div className="lg:col-span-5 bg-white border border-[#CCD5DF] rounded-xl overflow-hidden flex flex-col">
          <div className="p-4 border-b border-[#CCD5DF] space-y-3 bg-[#F8FAFC]">
            <div className="relative"><Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search name, phone or reception note..." className="w-full pl-9 pr-3 py-1.5 bg-white border border-[#CCD5DF] rounded-lg text-xs" /></div>
            <div className="flex bg-slate-100 p-0.5 rounded-lg border border-[#CCD5DF]">{(["All", "Active", "Inactive", "New"] as const).map(tab => <button key={tab} onClick={() => setFilter(tab)} className={`flex-1 py-1 text-xs font-bold rounded-md ${filter === tab ? "bg-white text-[#00685f] shadow-xs" : "text-slate-600"}`}>{tab}</button>)}</div>
          </div>
          <div className="divide-y divide-[#CCD5DF] overflow-y-auto flex-1">
            {filtered.map(contact => {
              const status = statusFor(contact);
              return <button key={contact.id} onClick={() => choose(contact)} className={`w-full p-4 text-left flex items-center justify-between border-l-4 ${contact.id === selectedId ? "bg-[#00685f]/[0.04] border-l-[#00685f]" : "border-l-transparent hover:bg-slate-50"}`}><span className="flex items-center gap-3 min-w-0"><span className="w-9 h-9 rounded-full bg-[#00685f]/15 text-[#00685f] font-bold text-xs flex items-center justify-center">{initials(contact.name)}</span><span className="min-w-0"><span className="text-sm font-bold text-[#0F172A] block truncate">{contact.name}</span><span className="text-xs text-slate-500">{contact.phone}</span></span></span><span className="text-right"><span className={`inline-block px-2 py-0.5 text-[10px] font-bold rounded-full border ${STATUS_COLORS[status]}`}>{status}</span><span className="text-[11px] text-slate-400 block mt-1">{contact.total_visits} visits</span></span></button>;
            })}
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-white border border-[#CCD5DF] rounded-xl p-6 overflow-y-auto">
          {selected ? <div className="space-y-6">
            <div className="flex items-start justify-between"><div className="flex items-center gap-4"><div className="w-12 h-12 rounded-full bg-[#00685f] text-white font-bold flex items-center justify-center">{initials(selected.name)}</div><div><h3 className="text-lg font-bold">{selected.name}</h3><button onClick={() => navigator.clipboard.writeText(selected.phone).then(() => addToast("Phone copied", "success"))} className="flex items-center gap-1 text-xs text-slate-500"><Phone size={12} /> {selected.phone} <Copy size={10} /></button></div></div><button onClick={() => addToast(`Open WhatsApp Inbox to message ${selected.name}`, "info")} className="px-3 py-1.5 bg-[#00685f] text-white text-xs font-bold rounded-lg flex items-center gap-1.5"><MessageCircle size={13} /> WhatsApp</button></div>
            <div className="grid grid-cols-2 gap-4"><div className="bg-[#F8FAFC] border border-[#CCD5DF] rounded-xl p-4"><span className="text-[10px] font-bold uppercase text-slate-500">Last recorded visit</span><p className="text-sm font-bold mt-1 flex items-center gap-1"><Calendar size={14} className="text-[#00685f]" /> {selected.last_visit ?? "No visit recorded"}</p></div><div className="bg-[#F8FAFC] border border-[#CCD5DF] rounded-xl p-4"><span className="text-[10px] font-bold uppercase text-slate-500">Lifetime visits</span><p className="text-sm font-bold mt-1">{selected.total_visits}</p></div></div>
            <div className="bg-[#F8FAFC] border border-[#CCD5DF] rounded-xl p-4 space-y-2"><div className="flex items-center justify-between"><span className="text-[10px] font-bold uppercase text-slate-500">WhatsApp permission</span><span className={`text-[10px] font-bold ${whatsappConsent?.status === "opted_in" ? "text-emerald-700" : "text-amber-700"}`}>{whatsappConsent?.status === "opted_in" ? "Opted in" : whatsappConsent?.status === "opted_out" ? "Opted out" : "Not recorded"}</span></div><textarea rows={2} value={consentEvidence} onChange={event => setConsentEvidence(event.target.value)} placeholder="Evidence, e.g. signed registration form dated..." className="w-full p-3 bg-white border border-[#CCD5DF] rounded-lg text-xs" /><div className="flex gap-2"><button onClick={() => saveConsent("opted_in")} className="flex-1 py-1.5 bg-emerald-700 text-white rounded-lg text-xs font-bold">Record opt-in</button><button onClick={() => saveConsent("opted_out")} className="flex-1 py-1.5 bg-white border border-[#CCD5DF] text-slate-700 rounded-lg text-xs font-bold">Record opt-out</button></div></div>
            <div className="space-y-2"><div className="flex items-center justify-between"><h4 className="text-[11px] font-bold uppercase text-slate-500">Reception notes</h4><button onClick={saveNote} className="px-3 py-1 bg-[#00685f] text-white text-xs font-bold rounded-md">Save</button></div><textarea rows={5} value={note} onChange={event => setNote(event.target.value)} placeholder="Language preference, callback request, accessibility or scheduling note..." className="w-full p-4 bg-[#F8FAFC] border border-[#CCD5DF] rounded-xl text-xs" /></div>
          </div> : <div className="h-full flex flex-col items-center justify-center text-slate-400 text-xs"><Users size={32} className="mb-2" /> Select a contact to view reception details.</div>}
        </div>
      </div>

      {showAdd && <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4"><div className="bg-white rounded-xl border border-[#CCD5DF] p-6 w-full max-w-md space-y-4"><div className="flex justify-between"><h3 className="font-bold">Add Patient Contact</h3><button onClick={() => setShowAdd(false)}><X size={16} /></button></div><input value={newName} onChange={event => setNewName(event.target.value)} placeholder="Full name" className="w-full px-3 py-2 border border-[#CCD5DF] rounded-lg text-xs" /><input value={newPhone} onChange={event => setNewPhone(event.target.value)} placeholder="WhatsApp number, e.g. +971..." className="w-full px-3 py-2 border border-[#CCD5DF] rounded-lg text-xs" /><div className="flex gap-2"><button onClick={() => setShowAdd(false)} className="flex-1 py-2 border border-[#CCD5DF] rounded-lg text-xs font-bold">Cancel</button><button onClick={addContact} className="flex-1 py-2 bg-[#00685f] text-white rounded-lg text-xs font-bold">Add Contact</button></div></div></div>}
    </div>
  );
}
